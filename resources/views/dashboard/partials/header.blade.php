        ...
        <div class="list-group-item bg-transparent">
            ...
            <div class="col">
                @foreach (Auth::guard('admin')->user()->notifications->take(5) as $notification)
                    <div class="py-2 border-bottom notification-item
                        {{ is_null($notification->read_at) ? 'bg-light' : '' }}"
                        style="border-radius: 6px;">

                        @if (isset($notification->data['message']) && str_contains($notification->data['message'], 'logged out'))
                            <!-- Logout Notification -->
                            <small>
    ...
    @endif
                    ...
                    <div class="modal-footer">
                        @if(Auth::guard('admin')->user()->notifications->count() > 0)
                            <form id="clearNotificationsForm" action="{{ url('/admin/notifications/clear') }}" method="POST">
                                @csrf
                                <button class="btn btn-danger btn-block">Clear All Notifications</button>
                            </form>
                        @else
                            <button class="btn btn-secondary btn-block" disabled>No Notifications</button>
                        @endif
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    notifIcon.addEventListener('click', function () {
        ...
        .then(data => {
            ...
            document.querySelectorAll('.notification-item.bg-light').forEach(el => {
